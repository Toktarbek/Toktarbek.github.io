<?php require_once $_SERVER['DOCUMENT_ROOT'].'/dostar/core/init.php';
if (!is_logged_in()) {
...
//Edit Category (Kategoryani ozgertu)
...
//Delete Category (Kategoryani oshiru)
if (isset($_GET['delete']) && !empty($_GET['delete'])) {
	$delete_id=(int)$_GET['delete'];
	$delete_id=sanitize($delete_id);
	$sql="SELECT * FROM categories WHERE id='$delete_id'"; //parentty oshirse astindagy kategoryasi birge oshetindey isteu
	$result=$db->query($sql);
...
if (isset($_POST) && !empty($_POST)) {
	...
	if ($category=='') {
		$errors[].='The categoy cannot by left blank. Kategoryani bos kaldiruga bolmaydy';
	}
	// If exists in the database, eger bazada bar bolsa
	if ($count>0) {
		$errors[].=$category. 'already exists. Plaese choose a new category.';
	}

	//Ddisplay Errors or Update Database (qatelikterdi korsetu nemese bazani zhukteu)
	if (!empty($errors)) {
		//Display errors(qatelikterdi korsetu)
