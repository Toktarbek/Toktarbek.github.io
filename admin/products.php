...
if (isset($_GET['add']) || isset($_GET['edit'])) {
	...
	if (isset($_GET['edit'])) {
		...
		//suretti oshirgende papkdende oshiru ushin
	...
	if ($_POST) {
		...
		if ($photoCount > 0) {
			for($i=0; $i<$photoCount; $i++){
				...
				$dbpath .='/dostar/images/products/'.$uploadName;
				if ($mimeType!='image') {
      ...
      <div class="container-fluid">
        <?php for($i=1; $i<=12; $i++): ?>
        	...
        	<div class="form-group col-md-2">
        		...
        		<input type="number" name="threshold<?=$i;?>" id="threshold<?=$i;?>" value="<?=((!empty($tArray[$i-1]))?$tArray[$i-1]:'');?>" min="0" 
        		class="form-control">
        	</div>
        <?php endfor; ?>
        </div>
